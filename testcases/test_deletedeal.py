...
class TestLead:
    @pytest.mark.parametrize("argvals", getcelldata("DeleteDeal", constants.XLS_FILEPATH))
    def test_deletedeal(self, argvals):
        #testcase - firstlevel check
        testrunmode = isrunnable("DeleteDeal", constants.XLS_FILEPATH)
                ...
                if(isinstance(homepage, Homepage)):
                    if(argvals['ExpectedResult']=='Success'):
                        ...
                        with allure.step("Deleting Deal....."):
                            leadhomepage.deletedeal(argvals['DealName'])
                    else:
