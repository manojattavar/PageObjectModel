...
class TestDeal:
    @pytest.mark.parametrize("argvals", getcelldata("CreateDeal", constants.XLS_FILEPATH))
                ...
                if(isinstance(homepage, Homepage)):
                    if(argvals['ExpectedResult']=='Success'):
                        ...
                        with allure.step("Creating Deal....."):
                            ...
                            dealcheck = enterdealdetails.leaddetails(argvals['DealName'], argvals['AccountName'], argvals['Stage'], argvals['ClosingDate'])
                    else:
